 ...
 */
export function assert(k6response, featureName, config, conditions, requestPayload) {
    const checks = {}
...
export function isExists(v, query) {
    ...
    try {
        ...
        for (let i = 0; i < splittedQuery.length; i++) {
            const query = splittedQuery[i];
            if (query === "[]") {
                if (Array.isArray(val)) {
                    val = val.map(item => item[splittedQuery[i + 1]]);
...
export function isEqual(v, query, expected) {
    ...
    try {
        ...
        for (let i = 0; i < splittedQuery.length; i++) {
            ...
            if (query === "[]") {
                if (Array.isArray(val)) {
                    val = val.find((p) => p[splittedQuery[i + 1]] == expected);
                    if (!val) {
                        return false;
                    }
                } else {
 ...
 * @param {number} minLength - The minimum length of the description.
 * @param {number} maxLength - The maximum length of the description.
 * @returns {string} - The generated random description.
 */
export function generateRandomDescription(maxLength) {
    const loremIpsum =
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.";

    const startIndex = generateRandomNumber(0, loremIpsum.length - maxLength);
    const description = loremIpsum.substring(startIndex, startIndex + maxLength);
